...
class Meetings extends CI_Controller{
	...
	{
		$data['meetings'] = $this->db->query("SELECT * FROM tbl_counsellor_student_schedule LEFT JOIN tbl_students ON tbl_counsellor_student_schedule.student_id = tbl_students.student_id WHERE tbl_counsellor_student_schedule.expert_id='".$this->session->userdata['expert_id']."' ORDER BY exp_std_date DESC ")->result();
