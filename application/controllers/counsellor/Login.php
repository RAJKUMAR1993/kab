...
class Login extends CI_Controller {
    ...
    public function index()
	{
		...
		$this->load->view('counsellor/login/login');
	}
	...
	public function do_login(){
        ...
        if ($this->form_validation->run() == FALSE) {
			...
			$data = $this->counsellor_login_model->login($email,$password);
	...
	public function forgot(){
		$this->load->view('counsellor/login/forgot_password');
	}
	

	public function reset_password(){
		$email = $this->input->post("email");
		$check = $this->db->where("expert_email",$email)->where("is_deleted",'0')->where("expert_status",'Active')->get("tbl_councellors")->num_rows();
		if($check > 0){
			$data['password']=$this->counsellor_login_model->api_key_crypt(random_string('alnum', 8),"e");
			$this->db->where("expert_email",$email)->update("tbl_councellors",array('password'=>$data['password']));
			$this->session->set_flashdata('msg',' <div class="alert alert-success" role="alert">Please check your email for login details.</div>');
			$msg = '<html><body><small>User ID : '.$email.'</small><br> <small>Password : '.$this->counsellor_login_model->api_key_crypt( $data["password"],"d").'</small></body></html>';
			...
			//redirect("counsellor/login/forgot");	
