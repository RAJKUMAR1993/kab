...
<? 
    $caldata = $this->db->select("tbl_expert_student_schedule.*")->join("tbl_experts", "tbl_experts.expert_id = tbl_expert_student_schedule.expert_id", "left")->where("tbl_expert_student_schedule.expert_id", $this->session->userdata("expert_id"))->where("tbl_experts.expert_status", "Active")->where("tbl_experts.is_deleted", 0)->get("tbl_expert_student_schedule")->result();
    $cdata = [];    
    if(count($caldata) > 0){
        foreach($caldata as $cl){
            $data = [];
            $data["name"] = $this->db->get_where("tbl_experts",["expert_id"=>$cl->expert_id])->row()->expert_name." (".$this->db->get_where("tbl_students",["student_id"=>$cl->student_id])->row()->student_name.")";
