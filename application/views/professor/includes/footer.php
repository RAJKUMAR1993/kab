...
<? 
    $caldata = $this->db->select("tbl_professor_meeting.*")->join("tbl_professors", "tbl_professors.pro_id = tbl_professor_meeting.session_pro_id", "left")->where("tbl_professor_meeting.session_pro_id", $this->session->userdata("pro_id"))->where("tbl_professors.pro_status", "Active")->where("tbl_professors.is_deleted", 0)->get("tbl_professor_meeting")->result();
    $cdata = [];    
    if(count($caldata) > 0){
        foreach($caldata as $cl){
            $data = [];
            $data["name"] = $this->db->get_where("tbl_professors",["pro_id"=>$cl->session_pro_id])->row()->pro_name." (".$this->db->get_where("tbl_students",["student_id"=>$cl->session_student_id])->row()->student_name.")";
            $data["date"] = $cl->session_date;
            $data["start"] = date('H:i:s', $cl->session_time);
            $data["end"] = '';
            $cdata[] = $data;
        }
    }   
    
	$webdata = $this->db->select("tbl_webinar_participants.*")->join("tbl_webinors", "tbl_webinors.id = tbl_webinar_participants.webinar_id", "left")->like("tbl_webinors.professor_ids", $this->session->userdata("expert_id"))->get("tbl_webinar_participants")->result();
    if(count($webdata) > 0){
        foreach($webdata as $wcl){
			 
			$wdata = $this->db->get_where("tbl_webinors",["id"=>$wcl->webinar_id])->row();
			
            $data = [];
            $data["name"] = $wdata->title." (".$this->db->get_where("tbl_students",["student_id"=>$wcl->webinar_student_id])->row()->student_name.")";
            $data["date"] = $wdata->date;
            $data["start"] = date('H:i:s', $wdata->from_time);
            $data["end"] = '';
            $cdata[] = $data;
        }
    }	
