...
<section id="ex">
  <div class="container emp-profile">
           ...
           <? echo $this->session->flashdata("payment_status"); ?>
            ...
            <form id="speakersession">
              <input type="hidden" name="login_std_mail" value="<?php if($this->session->userdata('email')) { echo $this->session->userdata('email'); } ?>">
              <input type="hidden" name="speid" value="<?php echo $sessions[0]->speaker_id; ?>" >
                <div class="row">
                    <div class="col-md-3">
                        <div class="profile-img">
                            <img src="<?php if($sessions[0]->speaker_photo == ""){ echo base_url().'assets/images/front/team4.png'; } else { echo base_url($sessions[0]->speaker_photo); } ?>" class="img-thumbnail img-fluid" alt=""/>
                    ...
                    <div class="col-md-7">
                        <div class="profile-head">
                                    <h5>
                                        <?php echo $sessions[0]->speaker_name; ?>
                                    </h5>
                                    <h6>
                                        <?php echo $sessions[0]->speaker_qualification; ?>
                                    </h6>
                                     <p class="proile-rating"><?php echo $sessions[0]->speaker_curorg; ?></p>
                                    
                                      <p>Language Spoken: <?php echo $sessions[0]->speaker_spokenlang; ?></p>
    ...
    <? } ?>
                            ...
                            <?php if(isset($sessions[0]->speaker_facebook) || isset($sessions[0]->speaker_twitter) || isset($sessions[0]->speaker_instagram)){ ?>
                            <h6>Follow Me On : </h6>
                            <?php if(isset($sessions[0]->speaker_facebook)){ ?>
                            <span><a href="<?php echo $sessions[0]->speaker_facebook; ?>" target="_blank"><img src="<?php echo base_url();?>assets/images/front/facebook.jpg"></a></span>
                            <?php }if(isset($sessions[0]->speaker_twitter)){ ?>
                            <span><a href="<?php echo $sessions[0]->speaker_twitter; ?>" target="_blank"><img src="<?php echo base_url();?>assets/images/front/twitter.jpg"></a></span>
                            <?php }if(isset($sessions[0]->speaker_instagram)){ ?>
                            <span><a href="<?php echo $sessions[0]->speaker_instagram; ?>" target="_blank"><img src="<?php echo base_url();?>assets/images/front/instagram.jpg"></a></span>
                            <?php }}?>
                          </div>
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="col-md-3">
                        <div class="profile-work">
                            <h3>Specialities</h3>
                           <?php echo $sessions[0]->speaker_expertise; ?>
                        </div>
                    </div>
                    <div class="col-md-9 cd">
                     ...
                     <h3>Biography</h3>
                                    <p><?php echo $sessions[0]->speaker_longdesc; ?></p>
       
                    <br/>
                    <?php if(isset($sessions[0]->speaker_awards) && $sessions[0]->speaker_awards!=''){ ?>
                    <h3>Awards/Recognitions</h3>
                    <p><?php echo $sessions[0]->speaker_awards; ?></p>
       
                    <br/>
                    <?php }if(isset($sessions[0]->speaker_targetgroup) && $sessions[0]->speaker_targetgroup!=''){ ?>
                    <h3>Target group</h3>
                    <p><?php echo $sessions[0]->speaker_targetgroup; ?></p>
       
                    <br/>
                    <?php }if(isset($sessions[0]->speaker_expparti) && $sessions[0]->speaker_expparti!=''){ ?>
                    <h3>Expectations from Participants</h3>
                    <p><?php echo $sessions[0]->speaker_expparti; ?></p>
            ...
            <li class="active"><a data-toggle="tab" href="#home" class="yourButtonClass" id="s30" session_duration="30" session_thirty="<?php  echo $sessions[0]->speaker_tcost; ?>" session_id="<?php  echo $sessions[0]->spe_se_id; ?>">30 Minute Session</a></li>
            <li><a data-toggle="tab" href="#menu1" class="yourButtonClass" id="s60" session_duration="60" session_thirty="<?php  echo $sessions[0]->speaker_scost; ?>" session_id="<?php  echo $sessions[0]->spe_se_id; ?>">60 Minute Session</a></li>
            </ul>
					<div class="tab-content">
					
					<div class="row tm">

                           <input type="hidden" name="speaker_session_id" id="sesid" value="<?php echo $sessions[0]->spe_se_id; ?>" />
                           <input type="hidden" name="sesduration" id="sesduration" value="30" />
                          <input type="hidden" name="sescost" id="sescost" value="<?php echo $sessions[0]->speaker_tcost; ?>" />
                          <input type="hidden" name="sesdate" id="sesdate" value="<?php echo $sessions[0]->spe_se_date; ?>" />
                          <input type="hidden" name="sestime" id="sestime" value="<?php echo $sessions[0]->spe_se_time; ?>" />
                    <div class="col-md-4"><h2 class="QA_Span">30 Minutes Career Q&A</h2></div>
                     <div class="col-md-4"><h2 style="float: right;"><span class="GFG_Span">Rs. <?php echo $sessions[0]->speaker_tcost; ?></span></h2></div>
                    </div>
                  ...
                  $checkexists = $this->db->select("*")->where("speaker_id", $sessions[0]->speaker_id)->where("spe_std_date", $se->spe_se_date)->where("spe_std_time", $se->spe_se_time)->where("spe_std_duration", $se->duration);
                  if($se->students_allowed == "multiple"){
                    $checkexists = $checkexists->where("student_id", $this->session->userdata("student_id"));
                  }
                  $checkexists = $checkexists->get("tbl_speaker_student_schedule");
                if($checkexists->num_rows()==0){
                ?>
							   <?php $d="0"; if($se->spe_se_date >= date("Y-m-d")){ if($se->duration==30){$d3++;}if($se->duration==60){$d6++;}$d="1"; ?>
							      <li data-id="<?php echo $se->duration.'_sessiontobehide';?>" class="<?php //if($k == '1'){ echo 'active'; } ?>" ><a href="#" class="yourButtonClass1" id="<?php  echo $se->spe_se_id; ?>" session_id="<?php  echo $se->spe_se_id; ?>" session_date="<?php  echo $se->spe_se_date; ?>" session_time="<?php  echo $se->spe_se_time; ?>"><?php echo $se->spe_se_date; ?> <?php echo date('H:i A', $se->spe_se_time); ?></a></li>
...
<!--               free sessions                -->
                  ...
                  $checkexistsf = $this->db->select("*")->where("speaker_id", $fsessions[0]->speaker_id)->where("spe_std_date", $fse->spe_se_date)->where("spe_std_time", $fse->spe_se_time)->where("spe_std_duration", $fse->duration);
                  if($fse->students_allowed == "multiple"){
                    $checkexistsf = $checkexistsf->where("student_id", $this->session->userdata("student_id"));
                  }
                  $checkexistsf = $checkexistsf->get("tbl_speaker_student_schedule");
                if($checkexistsf->num_rows()==0){
                ?>
							   <?php $df="0"; if($fse->spe_se_date >= date("Y-m-d")){ if($fse->duration==30){$d31++;}if($fse->duration==60){$d61++;}$df="1"; ?>
							      <li data-id="<?php echo $fse->duration.'_sessiontobehide';?>" class="<?php //if($k == '1'){ echo 'active'; } ?>" ><a href="#" class="yourButtonClass1" id="<?php  echo $fse->spe_se_id; ?>" session_id="<?php  echo $fse->spe_se_id; ?>" session_date="<?php  echo $fse->spe_se_date; ?>" session_time="<?php  echo $fse->spe_se_time; ?>"><?php echo $fse->spe_se_date; ?> <?php echo date('H:i A', $fse->spe_se_time); ?></a></li>
    ...
    <?php } ?>
      <div class="form-group">
         <? if($this->session->userdata("student_id")){ ?>
          <button type="submit" class="btn btn-danger round_btns book_now_sessions" data-sid="<? echo $this->session->userdata("speaker_id") ?>" <?php if($d == 0){ echo "disabled"; } ?> style="display: none;">Book now</button>
...
<script type="text/javascript">
    ...
    $("#speakersession").on('submit', function(e){
       e.preventDefault();
       var fdata = $("#speakersession").serialize();
       var url = '<?php echo base_url("front/speakers/save_speakersession")?>';
        //alert(url);
       $.ajax({
        url:url,
        data:fdata,
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <input type="hidden" name="institute_id" value="<? echo $sessions[0]->speaker_id; ?>">
