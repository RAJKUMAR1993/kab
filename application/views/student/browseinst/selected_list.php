								...
								<div class="col-lg-3">
									...
									<div class="file">		
										<div class="hover" style="z-index: 1000;">
											<a type="button" class="btn btn-icon btn-danger <? echo ($this->session->userdata("student_id")) ? 'removefromlist' :'' ?>" style="float: inline-end;" student_id="<? echo ($this->session->userdata("student_id")) ?>" institute_id="<? echo $in->institute_id; ?>" video_url="<?  echo $in->institute_video; ?>" style="font-size: 15px;color: white;cursor: pointer" data-toggle="<? echo ($this->session->userdata("student_id")) ? 'tooltip' : 'modal' ?>" title="Add To List" data-target="<? echo ($this->session->userdata("student_id")) ? '' : '#myModalLoginNow' ?>">
...
<script>
	$(".removefromlist").click(function(){
		 ...
		 $.ajax({
			type : "post",
			url : "<? echo base_url('student/browseinst/dislike') ?>",
			data : {
				student_id:sid,
				institute_id:iid,
				video_url:vurl
