...
<? 
    $caldata = $this->db->select("tbl_speaker_student_schedule.*")->join("tbl_speakers", "tbl_speakers.speaker_id = tbl_speaker_student_schedule.speaker_id", "left")->where("tbl_speaker_student_schedule.speaker_id", $this->session->userdata("speaker_id"))->where("tbl_speakers.speaker_status", "Active")->where("tbl_speakers.is_deleted", 0)->get("tbl_speaker_student_schedule")->result();
    $cdata = [];    
    if(count($caldata) > 0){
        foreach($caldata as $cl){
            $data = [];
            $data["name"] = $this->db->get_where("tbl_speakers",["speaker_id"=>$cl->speaker_id])->row()->speaker_name." (".$this->db->get_where("tbl_students",["student_id"=>$cl->student_id])->row()->student_name.")";
            $data["date"] = $cl->spe_std_date;
            $data["start"] = date('H:i:s', $cl->spe_std_time);
            $data["end"] = '';
            $cdata[] = $data;
        }
    }   
